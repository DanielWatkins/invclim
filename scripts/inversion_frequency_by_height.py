...
regcolors = {letter: color['color'] for letter, color in zip(['Eastern Eurasia',
                                                ...
                                                pplt.Cycle('538',9))}
fig, axs = pplt.subplots(nrows=6, ncols=8, journal='ams4', wspace=0.1, hspace=0.2)

arctic_stations['idx'] = np.arange(1, len(arctic_stations)+1)
season_plot = ['DJF', 'MAM', 'JJA', 'SON']
...
axs.format(xreverse=False, xlim=(-0.1,1.1),
           ylabel='Altitude (m)', xlabel='Frequency', abc=False, xtickminor=False, ytickminor=False, 
           xlocator=[0, 0.25, 0.5, 0.75, 1], xticklabels=['0', '', '0.5', '', '1'],
...
fig.save('../Images/Paper/freq_by_height_seasons.pdf')
